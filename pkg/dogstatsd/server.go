...
func (s *Server) Flush() {
	...
	s.demultiplexer.FlushAggregator(time.Now().Add(time.Second*10), true)
}
		...
		for {
			...
			case metricSampleType:
				...
				for idx := range samples {
					...
					batcher.appendSample(samples[idx]) // XXX(remy): use the Demultiplexer
					if s.histToDist && samples[idx].Mtype == metrics.HistogramType {
						...
						batcher.appendSample(*distSample) // XXX(remy): use the Demultiplexer
					}
				}
			}
		}
